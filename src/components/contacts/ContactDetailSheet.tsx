...
import { Contact, Company, Task } from '@/lib/types';
import { Mail, Phone, StickyNote, Briefcase, Building, User, Calendar, Zap, GitBranch, PlusCircle, CheckSquare } from 'lucide-react';
...
export function ContactDetailSheet({ contact, company, isOpen, onOpenChange }: ContactDetailSheetProps) {
  ...
  const contactTasks = tasks.filter(t => t.contactId === contact.id).sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime());
            ...
            <TabsContent value="tasks" className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div className="space-y-4">
                {contactTasks.length > 0 ? contactTasks.map((task) => {
                  ...
                  <p className="text-center text-sm text-momentum-dark-slate py-8">No tasks for this contact.</p>
                )}
              </div>
            </TabsContent>
